...
internal static class Statistics
{
    ...
    public static IEnumerable<Report> CreateReports()
    {
        ...
        for (var experimentCounter = 1; experimentCounter <= ExperimentsCount; ++experimentCounter)
        {
            ...
            var secondMatrix = Matrix.GenerateRandomMatrix(firstMatrixColumns, secondMatrixColumns);

            var experimentResults = GetExperimentsResults(firstMatrix, secondMatrix,
                MatrixMultiplication.SimpleMultiplyMatrix);
            var parallelExperimentResults = GetExperimentsResults(firstMatrix, secondMatrix,
                MatrixMultiplication.ParallelMultiplyMatrix);
            ...
            reports[experimentCounter - 1] = new Report(firstMatrixRows, firstMatrixColumns, firstMatrixColumns,
    ...
    public static void SaveReportsToFile(IEnumerable<Report> reports, string path)
    {
        ...
        table.DefaultCellBorder = new BorderInfo(BorderSide.All, .5f,
            Color.FromRgb(System.Drawing.Color.LightGray));
    ...
    private static long[] GetExperimentsResults(Matrix firstMatrix, Matrix secondMatrix, Func<Matrix, Matrix, Matrix> func)
    {
